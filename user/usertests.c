...
// each test a process and based on the exit_nomsg status of the process,
...
// does exit_nomsg() call iput(p->cwd) in a transaction?
void
exitiputtest(char *s)
{
  int pid, xstatus;
...
// what if two children exit_nomsg() at the same time?
void
twochildren(char *s)
{
...
// run each test in its own process. run returns 1 if child's exit_nomsg()
// indicates success.
int
